...
if [ -s data/localidades/rehacer.json ] && [ "$(tr -d '[] \n' < data/localidades/rehacer.json)" != "" ]; then
  ...
  # Lote de 20 y no de 50: el techo real es la cuota DIARIA de Groq —200.000
  # tokens por organización, unos 13.000 por ficha— así que pedir más solo
  # gasta intentos en balde. Las que fallen se van al final de la cola y les
  # toca turno la noche siguiente.
  npx tsx scripts/cron/salas-geo.mjs --rehacer --lote 20
else
